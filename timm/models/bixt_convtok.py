"""
BiXT Architecture Variants with convolutional tokeniser for Computer Vision applications;
...
Note: this script is partially based on the 'vision transformer' implementation by Ross Wightman / the timm library,
...
from timm.layers import ConvPatchEmbed
...
default_cfgs = {
    # Using the default 64 latents and 192 as embedding dimension
    'bixt_ti_cnv_l64_p16': _cfg(),

    # Using 1 more layer (d13), mainly as pretraining for dense image tasks like semantic segmentation
    'bixt_ti_cnv_l64_d13_p16': _cfg(),
    'bixt_ti_cnv_l64_d13_p8': _cfg(),
    'bixt_ti_cnv_l64_d13_p4': _cfg(),
}
...
class BiXTConvTokBlock(nn.Module):
    ...
    def __init__(
            ...
            sa_drop_path=0., ca_drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, ca_onesided=False,
...
class BiXTConvTok(nn.Module):
    """ Bidirectional Cross-Attention Vision Transformer architecture
    """
    def __init__(
            ...
            sa_drop_path_rate=0.1, ca_drop_path_rate=0.1, weight_init='jax', embed_layer=ConvPatchEmbed, norm_layer=None,
            act_layer=None, block_fn=BiXTConvTokBlock, latinit=False,
        ...
        # >> Single scale approaches to positional encoding:
        if embed_layer == ConvPatchEmbed:
            if posenc_type_pat == 'fourier2d_embadd_proj':
                # This is the same as before (e.g. fourier2d_embadd, etc.)
                self.patch_embed = ConvPatchEmbed(img_size=img_size, patch_size=patch_size,
                                                  in_chans=in_chans, embed_dim=embed_dim_pat)
            ...
            elif posenc_type_pat == 'fourier2d_embadd_proj_dyn':
                # This is the same as before (e.g. fourier2d_embadd, etc.)
                self.patch_embed = ConvPatchEmbed(img_size=img_size, patch_size=patch_size,
                                                  in_chans=in_chans, embed_dim=embed_dim_pat)
                # Using the posenc from XCiT, DETR, etc. -> fixed hidden dim that's upprojected to emb_dim;
    ...
    def init_weights(self, mode=''):
        ...
        # Initialising the learnt latent vectors
        if self.latinit:
...
def _create_vision_bixt_convtok(variant, pretrained=False, **kwargs):
    model = build_model_with_cfg(
        BiXTConvTok, variant, pretrained,
        **kwargs)
    return model
...
# ==================== Model definitions for convolutional tokeniser =====================================
@register_model
def bixt_ti_cnv_l64_p16(pretrained=False, **kwargs):
    """ Vision BiXT-Ti-conv (BiXT-conv-Ti/16)
    """
    model_kwargs = dict(patch_size=16, embed_dim_lat=192, embed_dim_pat=192, embed_dim_attn=192, depth=12,
                        ...
                        **kwargs)
    model = _create_vision_bixt_convtok('bixt_ti_cnv_l64_p16', pretrained=pretrained, **model_kwargs)
    return model


# 1 more layer, mainly for dense applications like semantic segmentation
@register_model
def bixt_ti_cnv_l64_d13_p16(pretrained=False, **kwargs):
    """ Vision BiXT-Ti-conv (BiXT-conv-d13-Ti/16)
    """
    model_kwargs = dict(patch_size=16, embed_dim_lat=192, embed_dim_pat=192, embed_dim_attn=192, depth=13,
                        num_latents=64, sa_num_heads=6, ca_num_heads=6, qkv_bias=True, rv_bias=True, latinit=True,
                        posenc_type_pat='fourier2d_embadd_proj', weight_init='jax',
                        **kwargs)
    model = _create_vision_bixt_convtok('bixt_ti_cnv_l64_d13_p16', pretrained=pretrained, **model_kwargs)
    return model


@register_model
def bixt_ti_cnv_l64_d13_p8(pretrained=False, **kwargs):
    """ Vision BiXT-Ti-conv (BiXT-conv-d13-Ti/8)
    """
    model_kwargs = dict(patch_size=8, embed_dim_lat=192, embed_dim_pat=192, embed_dim_attn=192, depth=13,
                        num_latents=64, sa_num_heads=6, ca_num_heads=6, qkv_bias=True, rv_bias=True, latinit=True,
                        posenc_type_pat='fourier2d_embadd_proj', weight_init='jax',
                        **kwargs)
    model = _create_vision_bixt_convtok('bixt_ti_cnv_l64_d13_p8', pretrained=pretrained, **model_kwargs)
    return model


@register_model
def bixt_ti_cnv_l64_d13_p4(pretrained=False, **kwargs):
    """ Vision BiXT-Ti-conv (BiXT-conv-d13-Ti/4)
    """
    model_kwargs = dict(patch_size=4, embed_dim_lat=192, embed_dim_pat=192, embed_dim_attn=192, depth=13,
                        num_latents=64, sa_num_heads=6, ca_num_heads=6, qkv_bias=True, rv_bias=True, latinit=True,
                        posenc_type_pat='fourier2d_embadd_proj', weight_init='jax',
                        **kwargs)
    model = _create_vision_bixt_convtok('bixt_ti_cnv_l64_d13_p4', pretrained=pretrained, **model_kwargs)
    return model
