...
def create_dataset(
    ...
    if name.startswith('torch/'):
        ...
        elif name == 'image_folder' or name == 'folder':  # OUR default in BiXT, given the available dataset structure
        ...
        assert has_imagenetv2, "Make sure to import imagenetv2 appropriately -- matched frequency version!"
        assert split in _EVAL_SYNONYM, "ImageNetv2 ONLY available for TESTING/Eval!"
        ds = ImageNetV2Dataset(root=root, **kwargs)
    else:
        # FIXME support more advanced split cfg for ImageFolder/Tar datasets in the future
        if search_split and os.path.isdir(root):
            # look for split specific sub-folder in root
            root = _search_split(root, split)
